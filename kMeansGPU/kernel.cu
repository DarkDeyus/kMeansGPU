...
template<typename T>
static T** init_d_array(unsigned int first_size, unsigned int second_size)
{
	T** result;
	CHECK_ERRORS(cudaMalloc(&result, first_size * sizeof(T*)));
...
template<typename T>
static void free_d_array(T** d_array, unsigned int first_size)
{
